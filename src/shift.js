...
class Shift {
  ...
  static manager (shiftsStore) {
    return new Shifts(shiftsStore)
  }
}

class Shifts {
  constructor (shiftsStore) {
    this.shiftsStore = shiftsStore
  }

  store (shift) {
    this.shiftsStore[shift.name] = shift
  }

  find (name) {
    return this.shiftsStore[name]
  }

  all () {
    return Object.entries(this.shiftsStore).map(([name, shift]) => shift)
  }
